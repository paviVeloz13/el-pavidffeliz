...
from helpers import create_jpeg, create_png, create_webp, pdf_page_count
...
class ImageOperationTests(unittest.TestCase):
    ...
    def test_webp_to_png_writes_png_and_preserves_source(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = create_webp(root / "sticker.webp")
            source_bytes = source.read_bytes()
            output = root / "sticker.png"

            result = convert_webp_to_png(source, output)

            self.assertEqual(result["operation"], "image.webp_to_png")
            self.assertEqual(result["output_paths"], [str(output)])
            self.assertEqual(source.read_bytes(), source_bytes)
            with Image.open(output) as image:
                self.assertEqual(image.format, "PNG")
    ...
    def test_webp_to_jpeg_flattens_alpha_and_applies_quality_level(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = create_webp(root / "badge.webp")
            output = root / "badge.jpg"

            result = convert_webp_to_jpeg(source, output, quality_level="high")

            self.assertEqual(result["operation"], "image.webp_to_jpeg")
            self.assertEqual(result["quality"], JPEG_QUALITY_LEVELS["high"])
            with Image.open(output) as image:
                self.assertEqual(image.format, "JPEG")
                self.assertEqual(image.mode, "RGB")
    ...
    def test_images_to_pdf_writes_one_page_per_image_in_order(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            third = create_webp(root / "third.webp", size=(25, 25))
            output = root / "images.pdf"

            result = images_to_pdf([first, second, third], output)
            ...
            self.assertEqual(result["sources"][2]["path"], str(third))
